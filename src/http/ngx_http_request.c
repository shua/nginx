...
ngx_http_process_request(ngx_http_request_t *r)
{
    ...
    if (r->http_connection->ssl) {
        ...
        if (ngx_ssl_verify_certificate(c, sscf->verify) != NGX_OK) {
            ngx_log_error(NGX_LOG_INFO, c->log, 0,
        ...
        if (ngx_ssl_verify_client_sent_certificate(c, sscf->verify) != NGX_OK) {
            ngx_log_error(NGX_LOG_INFO, c->log, 0,
        ...
        if (ngx_ssl_verify_ocsp(c, sscf->verify) != NGX_OK) {
            ngx_http_finalize_request(r, NGX_HTTPS_CERT_ERROR);
            return;
        }
    }
